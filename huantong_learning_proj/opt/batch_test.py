...
class BatchTestItem:
    ...
    opt_time = attr.ib()
    recorded_pred_downstream_id = attr.ib()
    recorded_update_time = attr.ib()


def prep_batch_test(batch_test_csv, output_pkl):
    batch_test_items = []
...
def clean_related_to_corrupt_update(config, tenant_id, batch_test_items):
    opt_times = []
    org_ids = set()
...
def clean_batch_test_items(
    postgres_config,
    tenant_id,
    batch_test_items_pkl,
    output_pkl,
    removed_output_folder,
):
...
def drop_pg_collations_after_opt_time_min(postgres_config, tenant_id, batch_test_items_pkl):
    config = PostgresConfig(**postgres_config)
...
def build_invalid_collation_ids(
    ...
    query_name_to_pg_collations = defaultdict(list)
    for pg_collation in tqdm(
        ...
        query_name_to_pg_collations[pg_collation.query_name].append(pg_collation)

    collation_ids = set()

    for pg_collations in query_name_to_pg_collations.values():
        invalid = False
        upstream_id_to_downstream_id = {}
        for pg_collation in pg_collations:
            if pg_collation.upstream_id not in upstream_id_to_downstream_id:
                upstream_id_to_downstream_id[pg_collation.upstream_id] = pg_collation.downstream_id
            if upstream_id_to_downstream_id[pg_collation.upstream_id] != pg_collation.downstream_id:
                invalid = True
                break
...
def stat_collation_id_and_timestamp(collation_id_and_timestamp_json):
...
def drop_pg_invalid_collations(
    postgres_config,
    tenant_id,
    invalid_collation_ids_jsons,
):
    ...
    config = PostgresConfig(**postgres_config)
    logging.info(
        'Dropping invalid collations according to invalid_collation_ids_jsons='
        f'{json.dumps(invalid_collation_ids_jsons,ensure_ascii=False,indent=2)}'
    )
...
class BatchTestRetrievedItem:
    batch_test_item = attr.ib()
    fast_pass = attr.ib()
    upstream_pg_org = attr.ib()
    retrieved_orgs = attr.ib()
...
def inspect_retrieval_failure(retrieved_items_pkl, output_json):
...
class BatchTestRankedItem:
    retrieved_item = attr.ib()
    sorted_retrieved_orgs = attr.ib()
    sorted_logits = attr.ib()
...
def run_batch_test_ranking(
    ...
    ranker_config = RankerConfig(**ranker_config)
    ranker_model, bert_tokenizer = load_ranker_model(ranker_config)
...
def run_batch_test_report(ranked_items_pkl, output_folder, target_thr=None):
    ...
    def dump_batch_test_item(batch_test_item):
        obj = attr.asdict(batch_test_item)
        obj = dump_datetime_attr(obj)
        return json.dumps(obj, indent=2, ensure_ascii=False)

    def dump_upstream_pg_org(upstream_pg_org):
        return dump_batch_test_item(upstream_pg_org)

    def dump_retrieved_org(retrieved_org):
        obj = attr.asdict(retrieved_org)
    ...
    def find_target_thr(ranked_items, expected_affirmative_ratio=0.93):
        import math
...
def run_batch_test_report_target_thrs(ranked_items_pkl):
    ranked_items = io.read_joblib(ranked_items_pkl)
    logger.info('Loaded.')
...
def run_e2e_batch_test_report(
    ...
    for batch_test_item, match in tqdm(zip(batch_test_items, matches)):
        ...
        result = {
            ...
            'upstream_id': batch_test_item.upstream_id,
            'upstream': dump_datetime_attr(attr.asdict(upstream)),
            'query': batch_test_item.query_name,
            'downstream_id': batch_test_item.downstream_id,
            'downstream': dump_datetime_attr(attr.asdict(downstream)),
        ...
        for place, pred_downstream_id in enumerate(match['match_downstream_ids']):
            if place <= pred_downstreams_max_load_place:
                ...
                dump_org = dump_datetime_attr(attr.asdict(pg_org))
                dump_aliases = [
...
def convert_results_json_to_xlsx(results_json, output_folder):
