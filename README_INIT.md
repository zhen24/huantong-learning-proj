...
fireball huantong_learning_proj.opt.prep_client_db_dump:convert_sql_to_csv \
    ...
    --output_csv="${HUANTONG_PROJ_DATA}/data/db-csv/20210615/ovalmaster.csv"

fireball huantong_learning_proj.opt.prep_client_db_dump:convert_sql_to_csv \
    --table_name="orgalias" \
    --sql_file="/share/dev/users/huntzhan/huantong/data/db-dump/20210615/ORGALIAS.sql" \
    --output_csv="${HUANTONG_PROJ_DATA}/data/db-csv/20210615/orgalias.csv"

fireball huantong_learning_proj.opt.prep_client_db_dump:convert_sql_to_csv \
    --table_name="orgcodecollate" \
    --sql_file="/share/dev/users/huntzhan/huantong/data/db-dump/20210615/ORGCODECOLLATE.sql" \
    --output_csv="${HUANTONG_PROJ_DATA}/data/db-csv/20210615/orgcodecollate.csv"
```
...
fireball huantong_learning_proj.opt.prep_client_db_dump:upload_orgs_to_pg \
    --ovalmaster_csv="${HUANTONG_PROJ_DATA}/release/db-csv/${DATABASE_VERSION}/ovalmaster.csv" \
    --tenant_id="$TENANT_ID" \
    --postgres_config="$POSTGRES_CONFIG"
...
fireball huantong_learning_proj.opt.prep_client_db_dump:upload_aliases_to_pg \
    --orgalias_csv="${HUANTONG_PROJ_DATA}/release/db-csv/${DATABASE_VERSION}/orgalias.csv" \
    --ovalmaster_csv="${HUANTONG_PROJ_DATA}/release/db-csv/${DATABASE_VERSION}/ovalmaster.csv" \
    --tenant_id="$TENANT_ID" \
    --postgres_config="$POSTGRES_CONFIG"
...
fireball huantong_learning_proj.opt.prep_client_db_dump:upload_collations_to_pg \
    --orgcodecollate_csv="${HUANTONG_PROJ_DATA}/release/db-csv/${DATABASE_VERSION}/orgcodecollate.csv" \
    --ovalmaster_csv="${HUANTONG_PROJ_DATA}/release/db-csv/${DATABASE_VERSION}/ovalmaster.csv" \
    --tenant_id="$TENANT_ID" \
    --postgres_config="$POSTGRES_CONFIG"
...
## 拷贝到 /home/zhen24/projects/huantong/HUANTONG_PROJ_DATA/release/db-csv/20210615 下作为初始化数据库时的数据快照
```bash
cd REQUIRED/
mv organization.csv alias.csv collation.csv /home/zhen24/projects/huantong/HUANTONG_PROJ_DATA/release/db-csv/20210615

# 下次初始化数据库时直接使用清洗后的快照数据，可跳过初始化时的清洗
cd /home/zhen24/projects/huantong/HUANTONG_PROJ_DATA/release/db-csv/20210615
